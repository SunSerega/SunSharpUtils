...
internal sealed partial class ExampleDataStash(String states_dir, CancellationToken svc_stop_token) : DataStash<ExampleDataStash, ExampleDataStash.TypedContent>(states_dir, svc_stop_token)
{
        ...
        , ITypedContentWithChildBlock<NetworkData.AddC, FileData.C, TypedContent.A?, TypedContent.C>
    {
        ...
        private readonly Dictionary<String, C> global_c = [];
        private readonly List<IGlobalContentModel> ordered_children = [];

        public Boolean TryGetParent(NetworkData.AddB data, [MaybeNullWhen(false)] out A result) =>
            this.all_a.TryGetValue(data.ParentId, out result);
        ...
        public static FileData.C ParseNetworkPacket(ExampleDataStash stash, NetworkData.AddC data) => data.Content;

        private void AddA(A a)
        {
            this.location_to_a.Add(a.CommonInfo.Location, a);
            this.all_a.Add(a.Id, a);
            this.ordered_children.Add(a);
        }
        public void AddC(C c)
        {
            this.global_c.Add(c.Id, c);
        ...
        {
            ITypedContent<TypedContent>.ValidateDictEqual("all_a", content1.all_a, content2.all_a, A.ValidateEqual);
            ITypedContent<TypedContent>.ValidateDictEqual("global_c", content1.global_c, content2.global_c, C.ValidateEqual);
        }
